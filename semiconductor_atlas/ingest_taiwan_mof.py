"""Import allowlisted Taiwan MOF active tax-registration records.

Each exact UBN remains a source-native tax-unit identity. A retained row is
evidence only that BGMOPEN1 published an active tax registration matching the
pinned semiconductor-candidate allowlist. It is not evidence of a legal
company, parent, owner, operator, head office, facility, operation, capacity,
or lifecycle state.
"""
...
IMPORTER_VERSION = "taiwan-mof-bgmopen1-tax-registration-import-v1"
SOURCE_FAMILY_KEY = "taiwan-mof-tax"
SOURCE_KEY = "taiwan-mof-tax:bgmopen1"
ENTITY_KEY_PREFIX = "taiwan-mof-tax:bgmopen1:"
SOURCE_FAMILY_NAME = "Taiwan MOF Active Tax Registration Registry"
SOURCE_FAMILY_DESCRIPTION = (
    "Allowlisted active tax-registration records retained as source-native "
    "organization-candidate evidence without legal-person or facility promotion."
)
SOURCE_NAME = "Taiwan MOF BGMOPEN1 active tax registrations"
SOURCE_PUBLISHER = "Taiwan Ministry of Finance, Fiscal Information Agency"
...
_SOURCE_NOTES = (
    "Exact source statement from Taiwan MOF BGMOPEN1. A UBN identifies a tax "
    "unit, not necessarily a legal company, parent, owner, operator, head office, "
    "or facility. Active-tax publication does not establish observed operation, "
    "capacity, output, utilization, yield, or lifecycle state."
)
_CLASSIFICATION = "semiconductor_tax_registration_candidate"
_SINGLE_FIELDS = (
    ("business_name", "organization.name"),
    ("business_address", "address.street"),
    ("unified_business_number", "organization.identifier.tw_ubn"),
    (
        "head_office_unified_business_number",
        "taiwan_mof_tax.head_office_unified_business_number",
    ),
    ("established_date_raw", "taiwan_mof_tax.established_date_raw"),
    ("organization_type", "taiwan_mof_tax.organization_type"),
)


@dataclass(frozen=True, slots=True)
class TaiwanMOFImportResult:
    source_family_id: str
    source_id: str
    matched_document_id: str
    allowlist_document_id: str
    raw_document_id: str
    ...
    tax_units_no_longer_matched: int
    organization_entity_ids: tuple[str, ...]
...
def _source_date(snapshot: VerifiedTaiwanMOFSnapshot) -> str:
    if snapshot.publisher_date_basis != ("first_csv_data_row_address_column_dd_mon_yy"):
        raise ValueError("Taiwan MOF publisher date basis drifted")
    return snapshot.publisher_date
...
def _atomic_import(connection: sqlite3.Connection) -> Iterator[None]:
    savepoint = f"taiwan_mof_import_{next(_SAVEPOINTS)}"
...
def _verify_snapshot_identity(
    supplied: VerifiedTaiwanMOFSnapshot,
    refreshed: VerifiedTaiwanMOFSnapshot,
) -> None:
    if supplied != refreshed:
        raise ValueError(
            "Taiwan MOF verified snapshot identity changed before database import"
        )


def _manifest_scope(snapshot: VerifiedTaiwanMOFSnapshot) -> dict[str, object]:
    try:
        manifest = json.loads(snapshot.manifest_bytes.decode("utf-8"))
        scope = manifest["source_scopes"][TAIWAN_MOF_SCOPE]
    except (KeyError, TypeError, UnicodeDecodeError, json.JSONDecodeError) as error:
        raise ValueError("Taiwan MOF verified manifest lacks source scope") from error
    if not isinstance(scope, dict):
        raise ValueError("Taiwan MOF verified source scope is invalid")
    return dict(scope)


def _ensure_document(connection: sqlite3.Connection, document: SourceDocument) -> bool:
    ...
    if conflicts:
        raise ValueError(
            f"Taiwan MOF source document {document.id} conflicts on "
            + ", ".join(conflicts)
        )
    return False


def _entity_key(business_number: str) -> str:
    return f"{ENTITY_KEY_PREFIX}{business_number}"
...
def _series_id(
    ...
    return stable_id("claim-series", entity_id, predicate, value_kind.value, dimension)
...
def _ensure_series(
    ...
    *,
    ...
    if existing is not None:
        ...
        if conflicts:
            raise ValueError(
                f"Taiwan MOF claim series {series_id} conflicts on "
                + ", ".join(conflicts)
            )
...
def _record_payload(record: TaiwanMOFTaxRecord) -> dict[str, object]:
    return {
        "dataset_id": "9400",
        "unified_business_number": record.unified_business_number,
        "row": dict(record.row),
    }


def _record_from_payload(payload_json: str, *, context: str) -> TaiwanMOFTaxRecord:
    try:
    ...
    business_number = payload.get("unified_business_number")
    row_value = payload.get("row")
    if (
        payload.get("dataset_id") != "9400"
        or not isinstance(business_number, str)
        or len(business_number) != 8
        or not business_number.isascii()
        or not business_number.isdecimal()
        or not isinstance(row_value, dict)
        or set(row_value) != set(TAIWAN_MOF_PUBLIC_FIELDS)
    ):
        raise ValueError(f"{context} payload identity drifted")
    row: dict[str, object] = {}
    for field in TAIWAN_MOF_PUBLIC_FIELDS:
        value = row_value[field]
        if field == "industry_activities":
            if not isinstance(value, list):
                raise ValueError(f"{context} industry payload drifted")
            activities: list[dict[str, str]] = []
            for activity in value:
                if (
                    not isinstance(activity, dict)
                    or set(activity) != {"code", "name"}
                    or not isinstance(activity["code"], str)
                    or len(activity["code"]) != 6
                    or not activity["code"].isascii()
                    or not activity["code"].isdecimal()
                    or not isinstance(activity["name"], str)
                    or not activity["name"]
                ):
                    raise ValueError(f"{context} industry payload drifted")
                activities.append({"code": activity["code"], "name": activity["name"]})
            row[field] = tuple(activities)
        elif isinstance(value, str):
            row[field] = value
        else:
            raise ValueError(f"{context} scalar payload drifted")
    if row["unified_business_number"] != business_number:
        raise ValueError(f"{context} payload business number drifted")
    return TaiwanMOFTaxRecord(
        business_number,
        row,
...
def _origin_record_row(
    ...
    *,
    ...
    if row is None:
        ...
        raise ValueError(
            f"Taiwan MOF replay lacks origin record for {entity_key}{suffix}"
        )
    return row
...
def _verified_origin_record(
...
) -> tuple[TaiwanMOFTaxRecord, str]:
    context = f"Taiwan MOF origin {row['id']}"
    try:
    ...
    if (
        parameters.get("source_updated_at_basis") != "http_last_modified_header"
        ...
        or row["id"] != stable_id("source-record", row["ingestion_run_id"], entity_key)
    ):
    ...
    if row["record_sha256"] != source_record_payload_sha256(payload) or str(
        row["payload_json"]
    ) != _canonical_json(payload):
        raise ValueError(f"{context} payload hash drifted")
    publisher_date = parameters.get("publisher_date")
    if not isinstance(publisher_date, str):
        raise ValueError(f"{context} publisher date drifted")
    try:
        valid_from = datetime.strptime(publisher_date, "%Y-%m-%d").date().isoformat()
    except ValueError as error:
        raise ValueError(f"{context} publisher date drifted") from error
    if valid_from != publisher_date:
        raise ValueError(f"{context} publisher date drifted")
...
def _source_created_at(connection: sqlite3.Connection, source_id: str) -> str:
    ...
    if row is None:
        raise ValueError("Taiwan MOF source has no ingestion run")
...
def _verify_entity_identity(
    ...
    *,
    ...
    expected = {
        "kind": EntityKind.ORGANIZATION.value,
        "stable_key": entity_key,
        "display_name": record.row["business_name"],
    ...
    if entity is None or any(entity[key] != value for key, value in expected.items()):
        raise ValueError(f"Taiwan MOF replay organization entity {entity_id} conflicts")
    ...
    return (
        EvidenceLink(
            ...
            excerpt=_canonical_json(
                {
                    "unified_business_number": business_number,
                    "field": field,
                    "value": value,
                    **({"token_index": token_index} if token_index is not None else {}),
                }
            ),
        ),
    )
...
def _source_specs(
    record: TaiwanMOFTaxRecord,
    *,
    matched_document_id: str,
    record_id: str,
) -> tuple[_ClaimSpec, ...]:
    row = record.row
    if row.get("unified_business_number") != record.unified_business_number:
        raise ValueError(
            f"Taiwan MOF record {record.unified_business_number} has mismatched identity"
        )
    ...
    for field, predicate in _SINGLE_FIELDS:
        ...
        if not isinstance(value, str):
            raise ValueError(
                f"Taiwan MOF record {record.unified_business_number} field {field} drifted"
            )
        if value == "":
            continue
        specs.append(
            _ClaimSpec(
                predicate,
                "tw_ubn" if predicate == "organization.identifier.tw_ubn" else "source",
                ScalarValue(ScalarType.TEXT, value),
                ClaimKind.SOURCE_STATEMENT,
                f"taiwan_mof_tax_{field}_exact_capture_v1",
                1.0,
                evidence=_field_evidence(
                    document_id=matched_document_id,
                    record_id=record_id,
                    business_number=record.unified_business_number,
                    field=field,
                    value=value,
                ),
                notes=_SOURCE_NOTES,
                identifier_metadata=(
                    ("TW_UBN", value, "TW")
                    if predicate == "organization.identifier.tw_ubn"
                    else None
                ),
            )
        )
    activities = row.get("industry_activities")
    if not isinstance(activities, tuple):
        raise ValueError(
            f"Taiwan MOF record {record.unified_business_number} industry activities drifted"
        )
    seen_activities: set[str] = set()
    for activity_index, activity in enumerate(activities):
        if not isinstance(activity, dict) or set(activity) != {"code", "name"}:
            raise ValueError(
                f"Taiwan MOF record {record.unified_business_number} industry activity drifted"
            )
        canonical_activity = _canonical_json(activity)
        if canonical_activity in seen_activities:
            continue
        seen_activities.add(canonical_activity)
        specs.append(
            _ClaimSpec(
                "taiwan_mof_tax.industry_activity",
                _token_dimension("industry_activity", canonical_activity),
                ScalarValue(ScalarType.TEXT, canonical_activity),
                ClaimKind.SOURCE_STATEMENT,
                "taiwan_mof_tax_industry_code_name_pair_exact_capture_v1",
                1.0,
                evidence=_field_evidence(
                    document_id=matched_document_id,
                    record_id=record_id,
                    business_number=record.unified_business_number,
                    field="industry_activities",
                    value=activity,
                    token_index=activity_index,
                ),
                notes=_SOURCE_NOTES,
            )
        )
...
def _derived_specs(
    ...
    *,
    ...
    dependency_id = source_claim_ids.get(
        ("organization.identifier.tw_ubn", record.unified_business_number)
    )
    if dependency_id is None:
        raise ValueError(
            f"Taiwan MOF record {record.unified_business_number} lacks exact UBN lineage"
        )
    return (
        _ClaimSpec(
            ...
            "taiwan_mof_bgmopen1_exact_ubn_allowlist_candidate_v1",
            1.0,
            evidence=(
                EvidenceLink(
                    allowlist_document_id,
                    locator=f"exact_ubn:{record.unified_business_number}",
                    excerpt=_canonical_json(
                        {
                            "match_rule": "exact_membership",
                            "unified_business_number": record.unified_business_number,
                        }
                    ),
                ),
            ...
            notes=(
                "Deterministic candidate classification from exact membership in the "
                "pinned UBN allowlist derived from verified MOENV and registered-factory "
                "snapshots. It does not equate this tax unit with a legal company, parent, "
                "owner, operator, head office, facility, or observed operation."
            ),
        ),
    )
    ...
    *,
    ...
    for row in rows:
        ...
        if series_id in claims:
            raise ValueError(
                f"Taiwan MOF series {series_id} has multiple active open claims"
            )
...
def _stored_value(connection: sqlite3.Connection, row: sqlite3.Row) -> ClaimValue:
    if row["value_kind"] != ValueKind.SCALAR.value:
        raise ValueError(
            f"Taiwan MOF claim {row['id']} has unsupported value kind {row['value_kind']}"
        )
    scalar = connection.execute(
        "SELECT * FROM scalar_values WHERE claim_version_id = ?", (row["id"],)
    ).fetchone()
    if scalar is None:
        raise ValueError(f"Taiwan MOF claim {row['id']} lacks its scalar value")
...
def _close_prior_claim(
    ...
    *,
    ...
    if _clock(str(prior["recorded_at"])) >= _clock(accepted_at):
        raise ValueError("Taiwan MOF refresh is not later than its prior claim")
    if str(prior["valid_from"]) > valid_to:
        raise ValueError("Taiwan MOF publisher date predates an active prior claim")
    connection.execute(
    ...
    created = insert_claim(
        ...
        ClaimVersion(
            ...
            "taiwan_mof_tax_source_interval_closure_v1",
            float(prior["confidence"]),
            valid_to=valid_to,
            created_by_run_id=run_id,
            notes=(
                "A later verified same-source active-tax snapshot closes only "
                f"the prior source-assertion interval at {valid_to}. {closure_basis}. "
                "Omission does not assert legal closure, tax-unit inactivity, company "
                "closure, facility lifecycle, operation, ownership, output, or capacity."
            ),
        ),
    ...
    _copy_identifier_metadata(connection, str(prior["id"]), closure_id)
...
def _assert_chronology(
    ...
    *,
    ...
    if later_acceptance is not None:
        raise ValueError(
            "Taiwan MOF snapshots must be imported in database-acceptance order"
        )
    for row in connection.execute(
        """
...
def _document_models(
    ...
    matched_id = stable_id(
        ...
        "privacy-conscious-allowlist-matched-derivative",
    )
    allowlist_id = stable_id(
        "source-document",
        source_id,
        TAIWAN_MOF_DATASET_URL,
        snapshot.retrieved_at,
        snapshot.allowlist_sha256,
        "source-bound-exact-ubn-allowlist",
    )
    raw_id = stable_id(
        "source-document",
        source_id,
    ...
    matched = SourceDocument(
        matched_id,
        source_id,
        TAIWAN_MOF_DATASET_URL,
        "Taiwan MOF BGMOPEN1 allowlist-matched active tax registrations",
        snapshot.retrieved_at,
        snapshot.matched_sha256,
    ...
    allowlist = SourceDocument(
        allowlist_id,
        source_id,
        TAIWAN_MOF_DATASET_URL,
        "Taiwan MOF exact semiconductor-candidate UBN allowlist",
        snapshot.retrieved_at,
        snapshot.allowlist_sha256,
    ...
    raw = SourceDocument(
        ...
        source_id,
        TAIWAN_MOF_ARCHIVE_URL,
        "Taiwan MOF BGMOPEN1 official active tax-registration archive",
        snapshot.retrieved_at,
        snapshot.raw_sha256,
        published_at=snapshot.source_updated_at,
        ...
        metadata={
            **static,
            ...
            "member_name": snapshot.scan.csv_member,
            "member_sha256": snapshot.scan.csv_sha256,
            "member_size": snapshot.scan.csv_bytes,
        },
    )
    return matched, allowlist, raw
...
def _apply_identifier_metadata(
    connection: sqlite3.Connection,
    claim_id: str,
    ...
    if metadata is None:
        if connection.execute(
            ...
            raise ValueError(
                f"Taiwan MOF claim {claim_id} has unexpected identifier metadata"
            )
        return
    scheme, normalized_value, jurisdiction = metadata
    add_organization_identifier_claim_metadata(
        connection,
        OrganizationIdentifierClaimMetadata(
            claim_id,
...
def _verify_identifier_metadata(
    connection: sqlite3.Connection,
    claim_id: str,
    ...
    row = connection.execute(
        ...
        FROM organization_identifier_claim_metadata
        WHERE claim_version_id = ?
        """,
        (claim_id,),
    ).fetchone()
    if expected is None:
        ...
        return
    actual = (
        None if row is None else str(row["scheme"]),
        None if row is None else str(row["normalized_value"]),
        None
        if row is None or row["jurisdiction"] is None
        else str(row["jurisdiction"]),
    )
    if actual != expected:
        raise ValueError(f"Taiwan MOF claim {claim_id} identifier metadata conflicts")


def _copy_identifier_metadata(
    connection: sqlite3.Connection,
    prior_claim_id: str,
    new_claim_id: str,
    ...
    metadata = (
        ...
        None if row["jurisdiction"] is None else str(row["jurisdiction"]),
    )
    _apply_identifier_metadata(connection, new_claim_id, metadata)
    return metadata
...
def _origin_context(
    ...
    *,
    ...
    allowlist_document = connection.execute(
        """
        SELECT source_document_id
        FROM ingestion_run_documents
        WHERE ingestion_run_id = ? AND role = 'allowlist'
        """,
        (ingestion_run_id,),
    ).fetchall()
    if len(allowlist_document) != 1:
        raise ValueError(
            f"Taiwan MOF origin run {ingestion_run_id} lacks one allowlist document"
        )
    context = _OriginContext(
        row,
        record,
        valid_from,
        str(allowlist_document[0]["source_document_id"]),
        _source_specs(
            record,
            matched_document_id=str(row["source_document_id"]),
...
def _expected_origin_claim(
    ...
    *,
    ...
    if claim is None:
        raise ValueError(f"Taiwan MOF replay lacks source claim {claim_version_id}")
    if claim["valid_to"] is not None:
        raise ValueError(
            f"Taiwan MOF origin claim {claim_version_id} is not an open assertion"
        )
    ...
    if len(matching_source) > 1:
        raise ValueError(
            f"Taiwan MOF origin claim {claim_version_id} has ambiguous source specs"
        )
    if matching_source:
        ...
        for source_spec in context.source_specs:
            ...
            if source_claim is None:
                raise ValueError(
                    f"Taiwan MOF origin lacks source series {source_series_id}"
                )
            ...
            _verify_exact_claim(
                connection,
                exact_source,
                context=f"Taiwan MOF origin source {source_claim['id']}",
            )
            assert isinstance(source_spec.value, ScalarValue)
            source_claim_ids[(source_spec.predicate, str(source_spec.value.value))] = (
                str(source_claim["id"])
            )
        matching_derived = [
            spec
            for spec in _derived_specs(
                context.record,
                source_claim_ids,
                allowlist_document_id=context.allowlist_document_id,
            )
            if _series_id(
                str(claim["subject_entity_id"]),
                spec.predicate,
        ...
        if len(matching_derived) != 1:
            raise ValueError(
                f"Taiwan MOF origin claim {claim_version_id} has no exact spec"
            )
    ...
    if claim_version_id != expected_id:
        raise ValueError(
            f"Taiwan MOF origin claim {claim_version_id} has nondeterministic identity"
        )
...
def _verify_exact_claim(
    ...
    *,
    ...
    try:
        ...
        _verify_identifier_metadata(
            connection, expected.version.id, expected.identifier_metadata
        )
    except ValueError as error:
...
def _verify_replay_claim_ledger(
    ...
    *,
    snapshot: VerifiedTaiwanMOFSnapshot,
    run: IngestionRun,
    matched_document: SourceDocument,
    allowlist_document: SourceDocument,
    complete_refresh: bool,
) -> None:
    ...
    for row in current_claims(connection, as_of=valid_from, recorded_at=run.started_at):
        if row["created_by_run_id"] not in source_run_ids:
            continue
        series_id = str(row["series_id"])
        if series_id in source_current:
            raise ValueError(
                f"Taiwan MOF replay has duplicate current series {series_id}"
            )
    ...
    def verify_spec(
        *,
        ...
        if (
            ...
            raise ValueError(f"Taiwan MOF replay claim series {series_id} conflicts")
        claim = source_current.get(series_id)
        if claim is None:
            raise ValueError(
                f"Taiwan MOF replay is missing expected claim series {series_id}"
            )
        dependencies = _claim_dependencies(connection, str(claim["id"]))
        if not _claim_matches(claim, spec, {str(claim["id"]): dependencies}):
            raise ValueError(f"Taiwan MOF replay claim {claim['id']} conflicts")
        if claim["created_by_run_id"] == run.id:
            ...
            if claim["id"] != expected_claim_id:
                raise ValueError(
                    f"Taiwan MOF replay claim {claim['id']} has nondeterministic identity"
                )
            if insert_claim(
                connection,
                ClaimVersion(
                ...
                raise AssertionError("Taiwan MOF replay unexpectedly inserted a claim")
            _verify_identifier_metadata(
                connection, expected_claim_id, spec.identifier_metadata
            )
            ...
            _verify_exact_claim(
                connection,
                exact,
                context=f"Taiwan MOF reused claim {claim['id']}",
            )
        return str(claim["id"])

    for record in snapshot.scan.records:
        entity_key = _entity_key(record.unified_business_number)
        ...
        for spec in _source_specs(
            record,
            matched_document_id=matched_document.id,
            record_id=record_id,
        ):
        ...
        for spec in _derived_specs(
            record,
            source_claim_ids,
            allowlist_document_id=allowlist_document.id,
        ):
            verify_spec(
                entity_id=entity_id,
                entity_key=entity_key,
    ...
    if complete_refresh and set(source_current) != expected_series_ids:
        raise ValueError("Taiwan MOF replay current same-filter claim set conflicts")
    if not expected_series_ids.issubset(source_current):
        raise ValueError("Taiwan MOF replay is missing current snapshot claims")
    ...
    for prior in superseded_priors:
        ...
        _verify_exact_claim(
            connection,
            exact_prior,
            context=f"Taiwan MOF superseded prior claim {prior['id']}",
        )
        if str(prior["valid_from"]) > valid_from:
            raise ValueError(
                "Taiwan MOF replay has a prior claim newer than its source update"
            )
        if str(prior["valid_from"]) == valid_from:
            continue
        closure_id = stable_id(
        ...
        expected_closure = _ExactClaim(
            ClaimVersion(
                ...
                notes=(
                    ...
                    f"{closure_basis}. Omission does not assert legal closure, tax-unit "
                    "inactivity, company closure, facility lifecycle, operation, ownership, "
                    "output, or capacity."
                ),
            ),
            exact_prior.value,
            evidence,
            dependencies,
            exact_prior.identifier_metadata,
        )
        _verify_exact_claim(
            connection,
            expected_closure,
            context=f"Taiwan MOF closure claim {closure_id}",
        )
    ...
    if actual_run_claim_ids != expected_run_claim_ids:
        raise ValueError(
            "Taiwan MOF replay run-created claim ledger conflicts "
...
def _existing_run_result(
    ...
    *,
    ...
    complete_refresh: bool,
    acceptance_timestamp_basis: str,
) -> TaiwanMOFImportResult:
    row = connection.execute(
        "SELECT * FROM ingestion_runs WHERE id = ?", (run.id,)
    ).fetchone()
    if row is None:
        raise AssertionError("Taiwan MOF replay requires an existing run")
    expected = {
        "source_id": run.source_id,
    ...
    if conflicts:
        raise ValueError(
            "Taiwan MOF replay conflicts with immutable ingestion run fields: "
            + ", ".join(conflicts)
        )
    ...
    expected_source = {
        ...
        "canonical_url": TAIWAN_MOF_DATASET_URL,
        "license": TAIWAN_MOF_LICENSE,
        "created_at": source_created_at,
    }
    if family is None or any(
        ...
        raise ValueError("Taiwan MOF replay source family conflicts")
    if source is None or any(
        source[key] != value for key, value in expected_source.items()
    ):
        raise ValueError("Taiwan MOF replay source conflicts")
    for document in (matched_document, allowlist_document, raw_document):
        existing = connection.execute(
            "SELECT * FROM source_documents WHERE id = ?", (document.id,)
        ).fetchone()
        expected_document = {
        ...
        if existing is None or any(
            ...
            raise ValueError(
                f"Taiwan MOF replay source document {document.id} conflicts"
            )
    expected_roles = {
        (matched_document.id, "primary"),
        (matched_document.id, "matched_derivative"),
        (matched_document.id, "source_record"),
        (allowlist_document.id, "allowlist"),
        (allowlist_document.id, "filter_input"),
    ...
    if actual_roles != expected_roles:
        raise ValueError("Taiwan MOF replay document lineage conflicts")
    ...
    if actual_records != expected_records:
        raise ValueError("Taiwan MOF replay source records conflict")
    _verify_replay_claim_ledger(
        connection,
        snapshot=snapshot,
        run=run,
        matched_document=matched_document,
        allowlist_document=allowlist_document,
        complete_refresh=complete_refresh,
    )
    errors = validate_database(connection)
    if errors:
        raise RuntimeError(
            "Taiwan MOF replay failed claim-store validation: " + "; ".join(errors)
        )
    final_snapshot = verify_taiwan_mof_allowlist_sources(
        snapshot.root, moenv_snapshot_dir, factory_snapshot_dir
    )
    _verify_snapshot_identity(snapshot, final_snapshot)
    return TaiwanMOFImportResult(
        ...
        tax_units_imported=snapshot.matched_count,
        ...
        tax_units_no_longer_matched=0,
        organization_entity_ids=tuple(entity_ids),
        source_record_ids=tuple(record_ids),
    )


def import_taiwan_mof_snapshot(
    connection: sqlite3.Connection,
    snapshot: VerifiedTaiwanMOFSnapshot,
    *,
    moenv_snapshot_dir: str | Path,
    ...
    """Import one immutable, source-bound BGMOPEN1 matched snapshot.

    A complete refresh closes prior same-source assertion intervals only. A
    partial refresh never closes omitted records or fields, and absence never
    becomes a claim about legal closure, tax inactivity, or facility lifecycle.
    """

    if not isinstance(snapshot, VerifiedTaiwanMOFSnapshot):
        raise TypeError("snapshot must be a VerifiedTaiwanMOFSnapshot")
    ...
    if acceptance_timestamp_basis not in ACCEPTANCE_TIMESTAMP_BASES:
        raise ValueError("Taiwan MOF import has an invalid acceptance timestamp basis")
    if _clock(accepted_at) < _clock(snapshot.retrieved_at):
        raise ValueError("Taiwan MOF accepted_at must not predate snapshot retrieval")
    if _clock(snapshot.source_updated_at) > _clock(snapshot.retrieved_at):
        raise ValueError("Taiwan MOF source_updated_at must not postdate retrieval")
    valid_from = _source_date(snapshot)
    family_id = stable_id("source-family", SOURCE_FAMILY_KEY)
    source_id = stable_id("source", SOURCE_KEY)
    matched_document, allowlist_document, raw_document = _document_models(
        snapshot, source_id
    )
    run_id = stable_id(
        "ingestion-run",
        IMPORTER_VERSION,
        matched_document.id,
        allowlist_document.id,
        raw_document.id,
        snapshot.manifest_sha256,
        snapshot.publisher_date,
        str(complete_refresh).casefold(),
    )
    manifest_scope = _manifest_scope(snapshot)
    parameters = {
        "accepted_at": accepted_at,
        "acceptance_timestamp_basis": acceptance_timestamp_basis,
        "active_tax_registration_row_count": snapshot.business_row_count,
        "allowlist": {
            "bytes": snapshot.allowlist_size,
            "derivation": snapshot.allowlist_derivation,
        ...
        "filter": manifest_scope.get("filter"),
        "filter_version": TAIWAN_MOF_FILTER_VERSION,
        "limitations": manifest_scope.get("limitations"),
        "manifest_sha256": snapshot.manifest_sha256,
        "missing_count": snapshot.missing_count,
        "organization_type_counts": dict(snapshot.organization_type_counts),
        "publisher_date": snapshot.publisher_date,
        "publisher_date_basis": snapshot.publisher_date_basis,
        "publisher_date_raw": snapshot.publisher_date_raw,
        "privacy": manifest_scope.get("privacy"),
        "raw_archive": {
            "bytes": snapshot.raw_size,
            "member": {
                "bytes": snapshot.scan.csv_bytes,
                "crc32": snapshot.scan.csv_crc32,
                "name": snapshot.scan.csv_member,
                "sha256": snapshot.scan.csv_sha256,
            },
            "path": snapshot.raw_path,
            "sha256": snapshot.raw_sha256,
        },
        "retrieval_timestamp_basis": snapshot.retrieval_timestamp_basis,
        "rights": manifest_scope.get("rights"),
        "snapshot_lineage": {
            "allowlist_source_bindings": snapshot.allowlist_derivation.get("sources"),
            "manifest_sha256": snapshot.manifest_sha256,
            "source_scope": TAIWAN_MOF_SCOPE,
        },
        ...
        "upstream_row_count_after_header_including_publisher_date": snapshot.row_count,
    }
    run = IngestionRun(
        run_id,
        source_id,
        ...
        input_document_id=matched_document.id,
    ...
    unchanged_reused = 0
    prior_closed = 0
    organization_entity_ids: list[str] = []
    ...
    with _atomic_import(connection):
        ...
        source_documents_created += int(_ensure_document(connection, matched_document))
        source_documents_created += int(
            _ensure_document(connection, allowlist_document)
        )
        source_documents_created += int(_ensure_document(connection, raw_document))
        add_ingestion_run(connection, run)
        add_ingestion_run_document(
            connection,
            IngestionRunDocument(run_id, matched_document.id, "primary"),
        )
        add_ingestion_run_document(
            connection,
            IngestionRunDocument(run_id, matched_document.id, "matched_derivative"),
        )
        add_ingestion_run_document(
            connection,
            IngestionRunDocument(run_id, matched_document.id, "source_record"),
        )
        add_ingestion_run_document(
            connection,
            IngestionRunDocument(run_id, allowlist_document.id, "allowlist"),
        )
        add_ingestion_run_document(
            connection,
            IngestionRunDocument(run_id, allowlist_document.id, "filter_input"),
        )
        add_ingestion_run_document(
            connection,
        ...
        def verify_prior(prior: sqlite3.Row) -> None:
            ...
            _verify_exact_claim(
                connection,
                expected,
                context=f"Taiwan MOF prior claim {prior_id}",
            )
            verified_prior_ids.add(prior_id)

        def close_prior(prior: sqlite3.Row, *, closure_basis: str) -> None:
        ...
        def write_spec(
            *,
            ...
            if prior is not None and _claim_matches(prior, spec, prior_dependencies):
                verify_prior(prior)
                unchanged_reused += 1
                return str(prior["id"])
            ...
            _apply_identifier_metadata(connection, claim_id, spec.identifier_metadata)
            claims_created += int(created)
            return claim_id
        ...
        for record in snapshot.scan.records:
            ...
            entity_id = stable_id("entity", entity_key)
            organization_entity_ids.append(entity_id)
            current_entity_ids.add(entity_id)
            display_name = record.row.get("business_name")
            if not isinstance(display_name, str) or not display_name:
                raise ValueError(
                    f"Taiwan MOF tax unit {record.unified_business_number} lacks a name"
                )
            entities_created += int(
                _ensure_entity(
                    connection,
        ...
        if errors:
            raise RuntimeError(
                "Taiwan MOF import failed claim-store validation: " + "; ".join(errors)
            )
        final_snapshot = verify_taiwan_mof_allowlist_sources(
            snapshot.root, moenv_snapshot_dir, factory_snapshot_dir
        )
    ...
    return TaiwanMOFImportResult(
        ...
        tax_units_no_longer_matched=len(prior_entity_ids - current_entity_ids),
        organization_entity_ids=tuple(organization_entity_ids),
